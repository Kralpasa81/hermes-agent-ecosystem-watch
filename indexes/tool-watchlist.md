...
- Current watch note: `v0.18.2` / `v2026.7.7.2` remains the latest tagged release; curated v0.19.0 release notes for the post-v0.18.0 window are still pending. On `main` (Jul 18), a genuine automation-reliability fix landed: `delegate_task` could silently drop subagent results in `hermes -z` (one-shot) runs and cron jobs, because both runners skip the async-delivery capability binding that lets a background delegation's completion re-enter the session — in the cron case, a dropped result could instead be misrouted into an unrelated session via an ambient session-key fallback. A new `declare_stateless_channel()` fixes both runners by routing `delegate_task` through the same inline/synchronous path the stateless HTTP adapter already uses. On Jul 17, a cluster of session auto-title race-condition fixes also landed: `set_auto_title_if_empty` now retries on collision instead of clobbering a concurrently-titled session, auto-title writes are atomic, and generation no longer overwrites a title a user set manually while generation was in flight. A genuine TUI reliability fix also shipped: a DECRQM-probing watchdog now recovers mouse tracking automatically when a terminal's own toggle (or tmux) clears DEC mouse modes, closing a deadlock that previously required a window resize to escape. Use `hermes update` or `pip install -U hermes-agent` to pick up tagged releases.
- Sources: https://github.com/NousResearch/hermes-agent/releases/tag/v2026.7.7.2, https://github.com/NousResearch/hermes-agent/commit/3d9be2789552a495c7adf30148e867e7614a4bdc, https://github.com/NousResearch/hermes-agent/commit/7facf63ae71b846d6079a69483fc95ce9aa8db75, https://github.com/NousResearch/hermes-agent/commit/46d16f4c2822722370168bd06a251ad5c193bebc
