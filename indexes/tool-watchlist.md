...
- Current watch note: `v2.1.186` is the latest stable release found today and is a meaningful workflow/safety update: CLI MCP login/logout commands, SSH-friendly `--no-browser` auth, background-subagent permission prompts surfaced in the main session, named-subagent allow/deny enforcement fixes, AWS credential refresh login support, bash-command auto-response behavior, and multiple background-agent reliability fixes.
