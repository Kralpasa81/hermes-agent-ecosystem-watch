...
- Current watch note: `v0.18.2` / `v2026.7.7.2` remains the latest tagged release; curated v0.19.0 release notes for the post-v0.18.0 window are still pending. On `main` (Jul 19), the gateway gained a **durable delivery-obligation ledger** for final responses (#67181): a response generated but not yet confirmed-delivered was previously the one artifact a gateway crash or planned restart could lose without a trace, forcing an expensive full-turn re-run on resume. The new `gateway/delivery_ledger.py` records every outbound final response in `state.db` before the first send attempt, sweeps dead-owner rows on startup, and redelivers with an explicit "♻️ Recovered reply — may be a duplicate" label rather than silently retrying. On Jul 18, a genuine automation-reliability fix landed: `delegate_task` could silently drop subagent results in `hermes -z` (one-shot) runs and cron jobs, because both runners skip the async-delivery capability binding that lets a background delegation's completion re-enter the session — in the cron case, a dropped result could instead be misrouted into an unrelated session via an ambient session-key fallback. A new `declare_stateless_channel()` fixes both runners by routing `delegate_task` through the same inline/synchronous path the stateless HTTP adapter already uses. Use `hermes update` or `pip install -U hermes-agent` to pick up tagged releases.
- Sources: https://github.com/NousResearch/hermes-agent/releases/tag/v2026.7.7.2, https://github.com/NousResearch/hermes-agent/commit/5854aad8b5, https://github.com/NousResearch/hermes-agent/commit/3d9be2789552a495c7adf30148e867e7614a4bdc
