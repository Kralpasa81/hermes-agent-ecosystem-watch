...
- Current watch note: `v0.18.2` / `v2026.7.7.2` remains the latest tagged release; curated v0.19.0 release notes for the post-v0.18.0 window are still pending. Nous Research (Hermes' maker) was reported (TechCrunch, Jul 13; AI Insider, Jul 14) to be in talks for a new funding round at a **$1.5B valuation** (≥$75M, led by Robot Ventures with USV participating) — unconfirmed by Nous directly. On `main` (Jul 15), a real correctness fix closed a bounded-capture regression that could **corrupt read-modify-write file operations over 50KB** by silently truncating reads feeding the patch engine; bounded capture is now opt-in and scoped only to the foreground terminal tool. Also landed: a delegation fail-closed fix preventing a fresh process from silently adopting a dead session's delegation results, and streaming-retry parity between the `anthropic_messages` and `chat_completions` call paths. Use `hermes update` or `pip install -U hermes-agent` to pick up tagged releases.
- Sources: https://github.com/NousResearch/hermes-agent/releases/tag/v2026.7.7.2, https://github.com/NousResearch/hermes-agent/commit/cab457d722a28d60cd90d3cd6c7e5b55a12b659a, https://github.com/NousResearch/hermes-agent/commit/47d853fdf26415fc36de10267512dca6fd2c6f91, https://techcrunch.com/2026/07/13/hermes-agent-maker-nous-research-in-talks-for-new-funding-at-1-5b-valuation/
...
- Current watch note: **`2.1.210`** (Jul 15) shipped security/reliability fixes: `isolation: 'worktree'` subagents can no longer run git-mutating commands against the main repo checkout instead of their own worktree; the Agent tool is now hardened against indirect prompt injection via content a subagent read; `ultracode` opt-in no longer fires on non-human-originated input (webhooks, relayed PR comments). `2.1.208` (first seen July 11–14) shipped a large reliability/performance pass: opt-in screen reader mode, vim insert-mode remaps, and multiple long-session memory-leak fixes (MCP stdio stderr capped, LSP documents LRU-capped at 50 docs, unbounded headless/SDK tool-result growth fixed); tool-heavy sessions get up to **7x faster tool rounds** via cached tool-pool assembly, and transcripts shrink up to 79x in edit-heavy sessions. `2.1.209` reverted an overly broad dialog-blocking guard in background sessions. Earlier: `2.1.207` made auto mode default on Bedrock/Vertex AI/Foundry (Opus 4.8 default).
