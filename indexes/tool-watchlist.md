...
- Current watch note: `2.1.196` is the latest release note found today. It is high-signal for organization default models, readable session names, clickable attachments, background-agent durability, Remote-session crash recovery, MCP OAuth scope handling for enterprise IdPs, safer MCP listing in untrusted workspaces, default-on streaming idle watchdog behavior, PowerShell search/diff parity, and reduced `/code-review` token usage. `2.1.195` remains important for extension safety and long-running agent operations: exact hook matching for hyphenated identifiers such as `mcp__brave-search`, explicit install consent for external plugins enabled only by project `.claude/settings.json`, multiple background-agent persistence/recovery fixes, remote-session startup provisioning checklist improvements, and voice-dictation fixes across macOS, Linux, and languages written without spaces.
...
- Current watch note: Codex Remote reached GA on June 25, enabling ChatGPT mobile to start or continue work on a connected Mac or Windows host with authenticated one-to-one QR pairing. The same changelog also notes a new DigitalOcean plugin for provisioning a Droplet and connecting it to Codex App as a remote workspace. Codex CLI `0.142.3` is the latest stable CLI release found today, but it is maintenance-only with no user-facing changes since `0.142.2`; the functional baseline to track remains `0.142.2` for MCP tool-search, proxy-aware auth, remote MCP, Bedrock recovery, and PowerShell approval behavior changes.
- Sources: https://developers.openai.com/codex/changelog, https://github.com/openai/codex/releases/tag/rust-v0.142.3, https://github.com/openai/codex/releases/tag/rust-v0.142.2
