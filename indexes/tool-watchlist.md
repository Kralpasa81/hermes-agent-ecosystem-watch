...
- Current watch note: `v0.18.2` / `v2026.7.7.2` remains the latest tagged release; curated v0.19.0 release notes for the post-v0.18.0 window are still pending. On `main` (Jul 16), two multiplex-gateway correctness fixes landed: secondary profiles in `multiplex_profiles` mode were starting their own Relay adapter instead of sharing the single process-level Relay ingress the active profile owns (now explicitly skipped for secondary profiles); and container-boot reconciliation was auto-starting named profile gateway slots from persisted "running" state even while the root gateway already multiplexes every profile, risking duplicate multiplex owners (named slots now stay registered but don't autostart under `GATEWAY_MULTIPLEX_PROFILES`). Also fixed: a Desktop reasoning-panel streaming flash for token-streaming reasoners, and (Jul 15) a compression fallback-timeout fix closing a per-turn multi-minute stall on large transcripts. Use `hermes update` or `pip install -U hermes-agent` to pick up tagged releases.
- Sources: https://github.com/NousResearch/hermes-agent/releases/tag/v2026.7.7.2, https://github.com/NousResearch/hermes-agent/commit/2ea39daeb1f675d72e5c21c9400f2d58d7e6d71a, https://github.com/NousResearch/hermes-agent/commit/6a35f9e667ec162489d14856e041f36ee63a12bf
...
- Current watch note: **`2.1.211`** (Jul 15) shipped mostly security/reliability hardening: `--forward-subagent-text` flag/env var to surface subagent text and thinking in stream-json output; permission-preview messages relayed to chat channels are now sanitized against bidirectional-override, zero-width, and look-alike quote characters (closes a visual approval-prompt spoofing vector); auto mode can no longer override a PreToolUse hook's `ask` decision for unsandboxed Bash; fixed a prompt-caching regression on Bedrock/Vertex/Mantle/Foundry that billed the trailing system context block as fresh input tokens on every request. **`2.1.210`** (Jul 15) closed a worktree-isolation bypass for subagents and hardened the Agent tool against indirect prompt injection. **`2.1.208`** (first seen July 11–14) shipped a large reliability/performance pass — up to **7x faster tool rounds** via cached tool-pool assembly, multiple long-session memory-leak fixes, and transcripts up to 79x smaller in edit-heavy sessions. Weekly rate limits were raised 50% through the Claude Fable 5 promo window (now through July 19), reportedly in response to the Codex/GPT-5.6 usage surge.
...
- Current watch note: **Codex + ChatGPT Work crossed 8M active users** (Jul 14–15) following the GPT-5.6 launch (Jul 9), which roughly doubled OpenAI's prior traffic peak within 48 hours. The surge forced real scaling rollbacks: the 5-hour usage cap was temporarily removed for Plus/Business/Pro; GPT-5.6 Sol's context window was cut back from 372k to 272k tokens after the larger window drained usage faster than intended; experimental reasoning-effort ("juice") changes were reverted; and usage-limit resets were pushed repeatedly, with some users still reporting missing banked resets as of Jul 16. CLI: `0.144.5` (Jul 16) improved dangerous-command detection (broader forced-`rm` coverage, clearer rejection reasons); `0.145.0` remains in alpha only. Net effect: Codex is under real capacity strain even as its user base grows fast — track usage-limit reliability, not just feature velocity.
- Sources: https://thenewstack.io/gpt-5-6-codex-user-surge/, https://community.openai.com/t/missing-banked-reset-after-codex-usage-limit-reset-announcement/1387097, https://github.com/openai/codex/releases
